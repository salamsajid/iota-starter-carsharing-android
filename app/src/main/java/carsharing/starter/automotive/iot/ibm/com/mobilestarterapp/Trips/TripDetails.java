...
public class TripDetails extends AppCompatActivity implements OnMapReadyCallback {
    ArrayList<Trip> stats = new ArrayList<Trip>();
    Trip trip;
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        String url = API.tripRoutes + "/" + tripData.trip_id;
        ...
        try {
            API.doRequest task = new API.doRequest(new API.doRequest.TaskListener() {
                ...
                public void postExecute(JSONArray result) throws JSONException {
                    ...
                    ArrayList<Path> stats = Path.fromDictionary(result);

                    if (stats.size() > 0) {
                        Path stat = stats.get(0);
    ...
    public void getDriverBehavior() {
        String trip_uuid = tripData.trip_uuid;

        if (trip_uuid == null) {
            String trip_id = tripData.trip_id;

            String url = API.tripAnalysisStatus + "/" + trip_id;
            ...
            try {
                API.doRequest task = new API.doRequest(new API.doRequest.TaskListener() {
                    ...
                    public void postExecute(JSONArray result) throws JSONException {
                        ...
                        if (result.length() > 0) {
                            JSONObject status = result.getJSONObject(0);
        ...
        String url = API.tripBehavior + "/" + trip_uuid;
            ...
            API.doRequest task = new API.doRequest(new API.doRequest.TaskListener() {
                ...
                public void postExecute(JSONArray result) throws JSONException {
                    ...
                    for (int i=0; i < result.length(); i++) {
                        Trip tempTripData = new Trip(result.getJSONObject(i));
                    ...
                    if (stats.size() > 0) {
                        ...
                        TripDetailsDataAdapter adapter = new TripDetailsDataAdapter(getApplicationContext(), behaviors);
